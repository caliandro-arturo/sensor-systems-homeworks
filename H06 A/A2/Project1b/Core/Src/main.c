...
void HAL_TIM_PeriodElapsedCallback (TIM_HandleTypeDef *htim){
	...
	if (HAL_I2C_Master_Receive(&hi2c1,LM75_ADDRESS+1,datas_temperature,6,20) == HAL_OK){
		...
		len = snprintf(str,sizeof(str),"Error reading the Sensor\r\n");
		HAL_UART_Transmit(&huart2,(uint8_t *)str,len,100);
	}
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MX_TIM2_Init();
  /* USER CODE BEGIN 2 */
  if (HAL_I2C_Master_Transmit(&hi2c1,LM75_ADDRESS,&LM75_TEMP_ADDRESS,1,20) != HAL_OK){
	  Error_Handler();
  }
